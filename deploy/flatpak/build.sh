...
if [[ "$arch" == "x86_64" ]]; then
	...
	do_configure "$X86_NO_OPENMP" --enable-simd=avx512 CPPFLAGS="-D_BOXED" && do_build ../run/john-avx512
	do_configure "$X86_REGULAR" --enable-simd=avx512 CPPFLAGS="-D_BOXED -DOMP_FALLBACK_BINARY=\"\\\"john-avx512\\\"\" -DCPU_FALLBACK_BINARY=\"\\\"john-avx2-omp\\\"\"" && do_build ../run/john-avx512-omp
